...
def main():
    ...
    path = ''
    model_load_path = os.path.join(path, rf'RL_mode{args.m}/best_model/model_{args.e}_mode_{args.m}_seed_{args.s}.pth')
    ...
    qtrain = qtrain1 + qtrain2 + qtrain3 + qtrain5 + qtrain6 + qtrain7 + qtrain8 + qtrain9 + qtrain10
    qvalid = qvalid1 + qvalid2 + qvalid3 + qvalid5 + qvalid6 + qvalid7 + qvalid8 + qvalid9 + qvalid10
    qtest = qtest1 + qtest2 + qtest3 + qtest5 + qtest6 + qtest7 + qtest8 + qtest9 + qtest10

    embeddings = generate_embeddings(qtrain, config, model_load_path)

    with open(os.path.join(path, rf'embeddings_mode_{args.m}_seed_{args.s}.pkl'), 'rb') as f:
