...
from RL_Training_ConnectFour.TreeSearch import MCTS
...
def take_turns(model, layer, probe, device, token_to_idx, mode):
    ...
    while True:
        for _ in range(50):
            tree.rollout(env.state)
        move_2 = tree.choose(env.state)[0].last_move
        _, done = env.step(move_2[1], -1)
...
def decisions_validate(probe_model_path, gpt_model_path, config, linear, seed, mode, layer, mcts, token_to_idx, random):
    ...
    if not random:
        with open('decision_outputs_vs_mcts.txt', 'a') as f:
            f.write(f'Winrate for probe mode {mode} seed {seed} layer {layer} linear {linear} mcts {mcts}: {success_rate}\n')
    else:
        with open('decision_outputs_random_vs_mcts.txt', 'a') as f:
