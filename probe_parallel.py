...
def standardize_tensor(tensor):
    """
    Normalize a 1-dimensional PyTorch tensor to the range [0, 1].

    Parameters:
    tensor (torch.Tensor): The input 1-dimensional tensor to normalize.
    ...
    # Avoid division by zero by clamping the range to a minimum value
...
def main():
    ...
    for s in tqdm(range(3)):
        for layer in tqdm(range(8)):
            ...
            values_train = [standardize_tensor(embedding[1][0]).to('cuda:0') for embedding in train if embedding[1][0].size()[0] == 7]
            del train
            embeddings_valid = [embedding[0].to('cuda:0') for embedding in valid if embedding[1][0].size()[0] == 7]
            values_valid = [standardize_tensor(embedding[1][0]).to('cuda:0') for embedding in valid if embedding[1][0].size()[0] == 7]
            del valid
            embeddings_test = [embedding[0].to('cuda:0') for embedding in test if embedding[1][0].size()[0] == 7]
            values_test = [standardize_tensor(embedding[1][0]).to('cuda:0') for embedding in test if embedding[1][0].size()[0] == 7]
            del test
