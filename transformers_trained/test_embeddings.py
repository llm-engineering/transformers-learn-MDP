...
class EpisodeDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        return torch.tensor(X_indices, dtype=torch.long), torch.tensor(list(Y_sequence.values()), dtype=torch.float32)
...
def main():
    ...
    start = int(len(qagent) * 0.9)

    for s in tqdm(range(3)):
        model_load_path = os.path.join(path, rf'RL_mode{args.m}/best_model/model_mode_{args.m}_seed_{s}.pth')
        for i in tqdm(range(8)):
            generate_embeddings(s, args.m, qagent[start:], config, model_load_path, token_to_idx, i)
