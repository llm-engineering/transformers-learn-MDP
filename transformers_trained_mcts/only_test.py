    ...
    for m in range(1, 2):
        for s in tqdm(range(3)):
            for layer in tqdm(range(8)):
                if os.path.exists(rf'layer{layer}seed{s}mode{m}.txt'):
                    if accelerator.is_main_process:
                        print(rf'layer {layer} complete')
                    continue
                total_embeddings = []
                for i in tqdm(range(8)):
                    with open(os.path.join(path, rf'seed_{s}_process_{i}_mode_{m}_layer_{layer}_embeddings.pkl'), 'rb') as f:
                        temp_embeddings = pickle.load(f)
                        total_embeddings.extend(temp_embeddings)
                embeddings_test = [embedding[0].to('cuda:0') for embedding in total_embeddings if embedding[1][0].size()[0] == 7]
                values_test = [embedding[1][0].to('cuda:0') for embedding in total_embeddings if embedding[1][0].size()[0] == 7]
                print(len(embeddings_test))
                probe_dataset_test = ProbeDataset(embeddings_test, values_test)
                del embeddings_test
                del values_test
                linear_probe_path = os.path.join(rf'mcts_mode{m}/probe_data', rf'Truebest_model_mode_{m}_seed_{s}_layer_{layer}.pth')
                nonlinear_probe_path = os.path.join(rf'mcts_mode{m}/probe_data', rf'Falsebest_model_mode_{m}_seed_{s}_layer_{layer}.pth')
                ...
                if accelerator.is_main_process:
                    with open('test_losses_mse.txt', 'a') as file:
                        file.write(f"Layer {layer} Seed {s} Mode {m} losses: {test_loss_linear}, {test_loss_nonlinear}")
                    with open(rf'layer{layer}seed{s}mode{m}.txt', 'w') as file:
                        file.write('a')
