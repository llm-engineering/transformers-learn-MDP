...
def train_probe(train_dataset : Dataset, valid_dataset : Dataset, epochs : int, params, linear, seed, accelerator):
    ...
    for epoch in tqdm(range(epochs)):
        ...
        best_model_path = str(linear) + f"best_model_seed_{seed}.pth"
        model_save_path = str(linear) + f"_model_{epoch+1}_seed_{seed}.pth"
        ...
        if accelerator.is_main_process:
            with open(str(linear) + f'_train_losses_seed_{seed}pkl', 'wb') as f:
                pickle.dump(train_loss_hist, f)
            with open(str(linear) + f'_valid_losses_seed_{seed}.pkl', 'wb') as f:
...
def test_probe(test_dataset, model_path, params, linear, accelerator):
    ...
    total_loss = torch.tensor(0.0).to(accelerator.device)
    total_loss2 = torch.tensor(0.0).to(accelerator.device)
    total_data = torch.tensor(0.0).to(accelerator.device)
    ...
    with torch.no_grad():
        for X, y in test_loader:
            ...
            # print(y)
            loss = criterion(preds, y)
            loss2 = criterion2(preds, y)
            total_loss += loss.item() * X.shape[0]
            total_loss2 += loss2.item() * X.shape[0]
            total_data += X.shape[0]

    accelerator.wait_for_everyone()

    total_loss = accelerator.gather(total_loss).sum()
    total_loss2 = accelerator.gather(total_loss2).sum()
    total_data = accelerator.gather(total_data).sum()

    return total_loss / total_data, total_loss2 / total_data
...
def main():
    ...
    for s in tqdm(range(3)):
        with open(os.path.join(path, rf'random_embeddings_mcts.pkl'), 'rb') as f:
        ...
        embeddings_train = [embedding[0].to('cuda:0') for embedding in train if embedding[1].size()[0] == 7]
        values_train = [embedding[1].to('cuda:0') * 5 for embedding in train if embedding[1].size()[0] == 7]
        del train
        embeddings_valid = [embedding[0].to('cuda:0') for embedding in valid if embedding[1].size()[0] == 7]
        values_valid = [embedding[1].to('cuda:0') * 5 for embedding in valid if embedding[1].size()[0] == 7]
        del valid
        embeddings_test = [embedding[0].to('cuda:0') for embedding in test if embedding[1].size()[0] == 7]
        values_test = [embedding[1].to('cuda:0') * 5 for embedding in test if embedding[1].size()[0] == 7]
        del test
        ...
        del embeddings_test
        del values_test
        linear_probe_path, _, _ = train_probe(probe_dataset_train, probe_dataset_valid, 100, (7, 512), True, s, accelerator)
        nonlinear_probe_path, _, _ = train_probe(probe_dataset_train, probe_dataset_valid, 100, (7, 512), False, s, accelerator)
        rank_loss_linear, test_loss_linear = test_probe(probe_dataset_test, linear_probe_path, (7, 512), True, accelerator)
        rank_loss_non_linear, test_loss_nonlinear = test_probe(probe_dataset_test, nonlinear_probe_path, (7, 512), False, accelerator)
        if accelerator.is_main_process:
            with open('test_losses.txt', 'a') as file:
                file.write(f"Random Seed {s} losses: {rank_loss_linear}, {rank_loss_non_linear}")
            with open('test_losses_mse.txt', 'a') as file:
                file.write(f"Random Seed {s} losses:: {test_loss_linear}, {test_loss_nonlinear}")
